#!/bin/bash
echo "DBS DAO приветствует Вас.Установка ноды запущена."
if [ ! $CELESTIA_NODENAME ]; then
	read -p "Введите имя Вашей ноды(произвольное,только буквы и цифры): " CELESTIA_NODENAME
fi
sleep 1
...
curl -s https://raw.githubusercontent.com/avatonk1/Celestia/main/inst_go.sh | bash &>/dev/null
